...
lines=$(echo $var | wc -l)
back_h=$(( 65 * lines + 60 ))
echo "\n\${image  /home/kushagra/.config/conky/conky_reminders/back.png  -p -1,-1 -s 260x$back_h}\n\${voffset -48}"
...
#   echo $line | sed 's/:.\{4\}//'
    if [[ $line != "" ]]; then
        ...
        echo -n "\n\${image ~/.config/conky/conky_calendar/strip.png  -p -6,$pos_y -s 260x68}${font1}\${offset -5}\${color $color1}\${color $color3}\${offset 10}\${voffset -2}$font2$timer\${goto 22}\${voffset 15}\${color #fff}$font3$task\${voffset 17}"
        pos_y=$(( pos_y + 57 ))
